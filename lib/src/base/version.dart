...
class Cache {
  ...
  static Cache get instance => context.get<Cache>() ?? Cache();
...
//    getStampFileFor(artifactName).writeAsStringSync(version);
    final Map<String, dynamic> stamp = jsonDecode(version);
    stamp.forEach((k, v) => Config.instance.setValue(k, v));
    }
...
//    return fs.file(fs.path.join(getRoot().path, '$artifactName.stamp'));
  ...
  String getVersionString({bool redactUnknownBranches = false}) {
    if (frameworkVersion != 'unknown')
      return '${getBranchName(
          redactUnknownBranches: redactUnknownBranches)}/$frameworkVersion';
    return '${getBranchName(
        redactUnknownBranches: redactUnknownBranches)}/$frameworkRevisionShort';
  }
  ...
  bool checkRevisionAncestry({
    ...
    final ProcessResult result = processManager.runSync(
      ...
      workingDirectory: Cache.flutterRoot, // todo
    );
  ...
  static String versionOutOfDateMessage(Duration frameworkAge) {
    String warning =
        'WARNING: your installation of Flutter is ${frameworkAge
        .inDays} days old.';
...
class VersionCheckStamp {
  ...
  @visibleForTesting
  static const String flutterVersionCheckStampFile = '.$kSettings';
...
class GitTagVersion {
  const GitTagVersion(this.x, this.y, this.z, this.hotfix, this.commits,
      this.hash);
