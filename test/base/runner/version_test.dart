  ...
  for (String channel in FlutterVersion.officialChannels) {
    ...
    group('cache', () {
      ...
      setUp(() {
        ...
        settingsFile = memoryFileSystem.file('.$kSettings');
        settingsFile.writeAsString(toPrettyJson({
          'version': '1.2.3',
          'stamp': '${DateTime.fromMicrosecondsSinceEpoch(0)}',
        }));
//      Cache.enableLocking();
      });
...
//      Cache.releaseLockEarly();
      ...
      testUsingContext('should get stamp', () async {
        final epoch = DateTime.fromMicrosecondsSinceEpoch(0, isUtc: true);
        VersionCheckStamp versionCheckStamp =
            VersionCheckStamp(lastTimeVersionWasChecked: epoch);
        await versionCheckStamp.store();
        versionCheckStamp = await VersionCheckStamp.load();
        print(versionCheckStamp.lastTimeVersionWasChecked);
        final cache = Cache();

        final stamp =
            cache.getStampFor(VersionCheckStamp.flutterVersionCheckStampFile);
        print(stamp);
        expect((DateTime.parse(jsonDecode(stamp)['lastTimeVersionWasChecked'])),
            epoch);
      }, overrides: <Type, Generator>{
        FileSystem: () => memoryFileSystem,
        Platform: () =>
            FakePlatform()..environment = {'HOME': fs.currentDirectory.path},
